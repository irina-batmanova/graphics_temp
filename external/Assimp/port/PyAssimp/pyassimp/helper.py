...
if os.name=='posix':
    ...
    additional_dirs.append('/usr/libmiptgl/')
    additional_dirs.append('/usr/libmiptgl/x86_64-linux-gnu')
    additional_dirs.append('/usr/local/libmiptgl/')
    ...
    if "conda" or "continuum" in sys.version.lower():
      ...
      pattern = re.compile('.*\/libmiptgl\/')
      conda_lib = pattern.match(cur_path).group()
      logger.info("Adding Anaconda libmiptgl path:"+ conda_lib)
      additional_dirs.append(conda_lib)

    # note - this won't catch libassimp.so.N.n, but
    # currently there's always a symlink called
    # libassimp.so in /usr/local/libmiptgl.
    ext_whitelist.append('.so')
    # libassimp.dylib in /usr/local/libmiptgl
...
def try_load_functions(library_path, dll):
    ...
    Arguments
    ---------
    library_path: path to current libmiptgl
